...
void printCardLine(const Card &card, const int line, const bool revealCard) {
	...
	if (line == 2) {
		const int cardNumber = card.getNumber() + 1; // Numer karty (1-based)

		if (card.isGuessed()) {
			cout << "|  " << ANSI_GREEN << (cardType.length() == 3
					 ? cardType + " "  : cardType.length() == 2
					 ? cardType + "  " : " " + cardType + "  ")
				<< ANSI_RESET;
			return;
		}
...
void BoardManager::printBoardWithRevealedCard(const Board &board, const int &cardNumber) {
	...
	for (int r = 0; r < rows; ++r) {
		printRow(cards, cols, r, cardNumber - 1);
		printLine(cols);
	}
}
	...
	for (int r = 0; r < rows; ++r) {
		for (int line = 0; line < 5; ++line) {
			for (int c = 0; c < cols; ++c) {
				...
				if (cards.contains(cardIndex)) {
					const bool revealCard = (cards.at(cardIndex).getNumber() == firstCardNumber - 1 ||
					                         cards.at(cardIndex).getNumber() == secondCardNumber - 1);
					printCardLine(cards.at(cardIndex), line, revealCard);
				} else {
					cout << "|      ";
				}
...
void BoardManager::printBoard(const Board &board) {
